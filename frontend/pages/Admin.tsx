...
import { admin as adminApi, estates as estatesApi } from "../api";
...
export default function Admin() {
  ...
  useEffect(() => {
    if (!token || user?.role !== "communest_admin") return;
    adminApi
      .allEstates(token)
      .then(setEstates)
      .catch(() => undefined);
  }, [token, user?.role, setEstates]);
      ...
      <Layout>
        <div className="flex items-center justify-center min-h-[60vh] text-center px-6">
          <div>
            ...
            >
              This page is only accessible to the Communest Admin.
            </p>
  ...
  const handleAddAdmin = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!addAdminEmail || !token) return;
    try {
      const result = await adminApi.addAdmin(token, addAdminEmail);
  ...
  const confirmDecision = async () => {
    if (!confirmId || !confirmAction || !token) return;
    try {
      const updatedEstate = await estatesApi.updateStatus(
        token,
        confirmId,
        confirmAction === "approve" ? "approved" : "denied",
      );
      setEstates((prev) =>
        prev.map((estate) =>
      ...
      setAddAdminMsg(
        requestError instanceof Error
          ? requestError.message
          : "Unable to update estate status.",
      );
    }
    setConfirmId(null);
    ...
    <Layout>
      ...
      <section
        ...
        style={{
          minHeight: "45vh",
          backgroundImage:
            'url("https://images.unsplash.com/photo-1497366216548-37526070297c?w=1600&h=900&fit=crop&auto=format")',
      ...
      >
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-6 py-20 text-center">
          ...
          >
            Manage estate approvals, platform admins, and overall platform
            health from this central dashboard.
          </p>
        </div>
      </section>
      ...
      <div className="max-w-5xl mx-auto px-6 py-12 flex flex-col gap-10">
        ...
        <section>
          ...
          <form
            ...
            className="card flex flex-col gap-4 max-w-lg"
          >
            <p className="text-sm" style={{ color: "var(--muted-foreground)" }}>
              Enter the email address of a Regular User account to grant them
              full Communest Admin status. This cannot be used on Estate Admin
              or Tenant accounts.
            </p>
        ...
        <section>
          ...
          <p
            ...
            style={{ color: "var(--muted-foreground)" }}
          >
            Review submitted estates and approve or deny them. Estate owners are
            notified by email upon decision.
          </p>
                      ...
                      <div className="flex items-start justify-between flex-wrap gap-3 mb-3">
                        <div>
                          ...
                          <p
                            ...
                            style={{ color: "var(--muted-foreground)" }}
                          >
                            {estate.location}, {estate.county} · {estate.units}{" "}
                            units · {estate.totalArea.toLocaleString()} m²
                          </p>
                        </div>
                        <p
                          className="text-xs"
                        ...
                        >
                          Submitted:{" "}
                          {new Date(estate.submittedAt).toLocaleDateString()}
                        </p>
                      </div>
      ...
      {confirmId && confirmAction && (
        ...
        >
          <div
            className="w-full max-w-sm rounded-2xl p-7 animate-fade-in"
            style={{
              background: "var(--card)",
